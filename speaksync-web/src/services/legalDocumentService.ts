/**
 * Firebase Legal Documents Service for Web
...
class LegalDocumentService implements LegalDocumentAdmin, LegalDocumentReader {
  ...
  async getActiveDocument(type: LegalDocumentType): Promise<LegalDocument | null> {
    try {
      ...
      console.warn('Error fetching active document:', error);
      return null;
    }
  }
  ...
  async getAllActiveDocuments(): Promise<LegalDocument[]> {
    try {
      ...
      return snapshot.docs.map(docSnap => ({
        id: docSnap.id,
        ...docSnap.data(),
        effectiveDate: docSnap.data().effectiveDate?.toMillis?.() || docSnap.data().effectiveDate,
        createdAt: docSnap.data().createdAt?.toMillis?.() || docSnap.data().createdAt,
        updatedAt: docSnap.data().updatedAt?.toMillis?.() || docSnap.data().updatedAt
      })) as LegalDocument[];
    } catch (error) {
      console.warn('Error fetching all active documents:', error);
      return [];
    }
  }
  ...
  async getDocumentByVersion(type: LegalDocumentType, version: string): Promise<LegalDocument | null> {
    try {
      ...
      console.warn('Error fetching document by version:', error);
      return null;
    }
  }
  ...
  async createNewVersion(
    ...
    try {
      ...
      console.warn('Error creating new version:', error);
  ...
  async activateVersion(documentId: string, version: string): Promise<void> {
    try {
      ...
      console.warn('Error activating version:', error);
  ...
  async deactivateDocument(documentId: string): Promise<void> {
    try {
      ...
      console.warn('Error deactivating document:', error);
  ...
  async getDocumentHistory(documentId: string): Promise<LegalDocumentHistory> {
    try {
      ...
      console.warn('Error fetching document history:', error);
  ...
  async recordUserAcceptance(acceptance: Omit<UserLegalAcceptance, 'isCurrentVersion'>): Promise<void> {
    try {
      ...
      console.warn('Error recording user acceptance:', error);
  ...
  async getUserLegalStatus(userId: string): Promise<UserLegalStatus> {
    try {
      ...
      const acceptances = acceptancesSnapshot.docs.map(docSnap => ({
        ...docSnap.data(),
        acceptedAt: docSnap.data().acceptedAt?.toMillis?.() || docSnap.data().acceptedAt
      })) as UserLegalAcceptance[];

      // Get all active documents to check compliance
      const activeDocuments = await this.getAllActiveDocuments();
      const requiredTypes = activeDocuments.map(document => document.name as LegalDocumentType);
      ...
      console.warn('Error fetching user legal status:', error);
